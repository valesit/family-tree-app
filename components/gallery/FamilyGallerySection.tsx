...
/** Built-in suggested categories for organizing authentic family photos. */
...
type UploadRow = {
  ...
  url: string;
  label: string;
  /** Built-in category id OR a user-coined slug. NULL = "Uncategorized". */
...
/** 'all' | builtin id | 'uncategorized' | `custom:<slug>`. */
type FilterId = 'all' | GalleryCategoryId | 'uncategorized' | `custom:${string}`;
...
export function FamilyGallerySection({ rootPersonId, compact }: FamilyGallerySectionProps) {
  ...
  const filteredUploads = useMemo(() => {
    ...
    if (activeFilter === 'uncategorized') return uploads.filter((u) => !u.category);
  ...
  const isEmpty = !isLoading && filteredUploads.length === 0;
  ...
  return (
    ...
    >
      ...
      <div className="relative mb-4 flex flex-col gap-3 sm:mb-5 sm:flex-row sm:items-end sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-xs text-slate-600 sm:mt-1 sm:text-sm">
            Authentic photos shared by family members. Sign in to add your own.
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-dashed border-slate-200 bg-white/60 py-12 text-center">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              {isAuthenticated
                ? 'Tap “Add photo” to share an authentic family memory.'
                : 'Sign in to be the first to share a photo.'}
            </p>
          </div>
        </div>
      ) : (
        <ul className="m-0 grid list-none grid-cols-2 gap-2.5 p-0 sm:gap-3 md:grid-cols-3 lg:grid-cols-4">
          {filteredUploads.map((row, idx) => (
